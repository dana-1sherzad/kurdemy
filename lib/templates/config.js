...
jobs:
  test:
    ...
    services:
      ${config.database !== 'sqlite' ? `${config.database}:
        image: ${getDatabaseImage(config.database)}
        env:
          ${getDatabaseEnv(config.database)}
        options: >-
          --health-cmd "${getDatabaseHealthCheck(config.database)}"
          --health-interval 10s
          --health-timeout 5s
          --health-retries 5
        ports:
    ...
    - name: Set up database
      run: |
        ${config.database === 'sqlite' ? 'touch dev.db' : `npm run db:push`}

    - name: Run tests
      run: npm test
      env:
        CI: true
        DATABASE_URL: ${getDatabaseTestUrl(config.database)}
...
${config.database !== 'sqlite' ? '   2. Configure your database connection' : ''}
...
`;
  ...
  const dbResetScriptContent = `#!/bin/bash
...
npm run db:seed
` : `
# Reset Drizzle database
${config.database === 'sqlite' ? 'rm -f dev.db && touch dev.db' : 'npm run db:migrate'}
...
`;
  ...
  await fs.writeFile(path.join(projectPath, 'scripts/db-reset.sh'), dbResetScriptContent);
  await fs.chmod(path.join(projectPath, 'scripts/db-reset.sh'), '755');
...
${config.orm === 'prisma' ? 'npx prisma db push --accept-data-loss > /dev/null 2>&1' : 'echo "Database check skipped for Drizzle"'}
if [ $? -eq 0 ]; then
...
function getDatabaseHealthCheck(database) {
  const checks = {
    ...
    sqlserver: '/opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "YourStrong@Passw0rd" -Q "SELECT 1"'
  };
...
function getDatabaseTestUrl(database) {
  const urls = {
    postgresql: 'postgresql://postgres:postgres@localhost:5432/test_db',
    mysql: 'mysql://root:root@localhost:3306/test_db',
    sqlserver: 'sqlserver://localhost:1433;database=test_db;username=sa;password=YourStrong@Passw0rd;trustServerCertificate=true',
    sqlite: 'file:./test.db'
  };
